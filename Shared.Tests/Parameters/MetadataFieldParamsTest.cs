...
namespace CloudinaryDotNet.Test.Parameters
{
    public class MetadataFieldParamsTest
    {
        ...
        public void TestIntMetadataFieldCreateParamsCheck()
        {
            ...
            Assert.Throws<ArgumentException>(parameters.Check, forbiddenValidationTestMessage);

            parameters.Validation = new DateGreaterThanValidationParams(DateTime.MinValue);
            Assert.Throws<ArgumentException>(parameters.Check, forbiddenValidationTestMessage);

            parameters.Validation = new DateLessThanValidationParams(DateTime.MaxValue);
            Assert.Throws<ArgumentException>(parameters.Check, forbiddenValidationTestMessage);
        }
        ...
        public void TestStringMetadataFieldCreateParamsCheck()
        {
            ...
            parameters.Validation = new IntGreaterThanValidationParams(10);
            Assert.Throws<ArgumentException>(parameters.Check, forbiddenValidationTestMessage);

            parameters.Validation = new IntLessThanValidationParams(10);
            Assert.Throws<ArgumentException>(parameters.Check, forbiddenValidationTestMessage);
        ...
        public void TestEnumMetadataFieldCreateParamsCheck()
        {
            ...
            Assert.Throws<ArgumentException>(parameters.Check, emptyLabelTestMessage);

            parameters.Label = label;
            parameters.Mandatory = true;
            Assert.Throws<ArgumentException>(parameters.Check, emptyDefaultValueTestMessage);

            parameters.DefaultValue = defaultString;
            Assert.Throws<ArgumentException>(parameters.Check, emptyDataSourceTestMessage);
            ...
            parameters.Validation = new StringLengthValidationParams();
            Assert.Throws<ArgumentException>(parameters.Check, nonEmptyValidationTestMessage);
        }
        ...
        public void TestIntMetadataFieldUpdateParamsCheck()
        {
            ...
            Assert.Throws<ArgumentException>(parameters.Check, nonEmptyDataSourceTestMessage);
        ...
        public void TestMetadataDataSourceParamsCheck()
        {
            ...
            Assert.Throws<ArgumentException>(parameters.Check, emptyValuesTestMessage);

            parameters.Values = new List<EntryParams>();
            Assert.Throws<ArgumentException>(parameters.Check, emptyValuesTestMessage);

            parameters.Values.Add(new EntryParams(null));
            Assert.Throws<ArgumentException>(parameters.Check, "Must be specified Value");
        }
        ...
        public void TestStringLengthValidationParamsCheck()
        {
            ...
            Assert.Throws<ArgumentException>(parameters.Check, "Either min or max must be specified");

            parameters.Min = -5;
            Assert.Throws<ArgumentException>(parameters.Check, "Must be a positive integer Min");

            parameters.Min = 10;
            parameters.Max = -10;
            Assert.Throws<ArgumentException>(parameters.Check, "Must be a positive integer Max");
        }
        ...
        public void TestAndValidationParamsCheck()
        {
            ...
            Assert.Throws<ArgumentException>(parameters.Check, "Rules list must not be null");

            parameters.Rules = new List<MetadataValidationParams>();
            Assert.Throws<ArgumentException>(parameters.Check, "Rules list must not be empty");
        }

        [Test]
        public void TestDeleteDataSourceEntriesParamsCheck()
        {
            var parameters = new DeleteDataSourceEntriesParams(new List<string> {"some value"})
            {
                ExternalIds = null
            };
            emptyExternalIdsTestMessage = "Must be specified ExternalIds";
            Assert.Throws<ArgumentException>(parameters.Check, emptyExternalIdsTestMessage);
        ...
        [Test]
        public void TestDeleteDataSourceEntriesParamsDictionary()
        {
            const string externalId1 = "id 1";
            const string externalId2 = "id 2";
            var externalIds = new List<string> {externalId1, externalId2};
            var parameters = new DeleteDataSourceEntriesParams(externalIds);
